...
struct Window {
    ...
    Window(vk::raii::Instance const &instance, std::string_view windowName, vk::Extent2D const &extent)
            : windowName{windowName} {

        glfw::GLFWwindow *windowRaw = glfw::glfwCreateWindow(
                static_cast<int>(extent.width), static_cast<int>(extent.height),
                std::string{windowName}.data(), // Enforce null-termination
                nullptr, nullptr
        );
        windowHandle = {windowRaw, glfw::glfwDestroyWindow};

        vkc::VkSurfaceKHR rawSurface;
        if (auto result = static_cast<vk::Result>(glfw::glfwCreateWindowSurface(
                    *instance, windowRaw, nullptr, &rawSurface
...
vk::SurfaceFormatKHR pickSurfaceFormat(std::span <vk::SurfaceFormatKHR> formats) {
    ...
    auto it = std::ranges::find(formats,
...
struct Swapchain {
    ...
    Swapchain(vk::raii::Device const &device, vk::raii::PhysicalDevice const &physicalDevice,
